...
export default {
  ...
  mounted: function() {
    if (this.name) {
      fetch(`http://${process.env.VUE_APP_API_URL}/api/departments/${this.name}/students`)
        .then(response => response.json())
        .then(data => (this.students = data));
    }
    // get department id
    fetch(`http://${process.env.VUE_APP_API_URL}/api/departments/${this.name}`)
      .then(response => response.json())
      .then(data => (this.currentDepartment = data));
  },
  methods: {
    async addStudent() {
      await fetch(`http://${process.env.VUE_APP_API_URL}/api/students/`, {
        method: "POST",
        headers: {
